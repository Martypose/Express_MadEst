...
function wireNamespace(nsp, nsName = '/') {
  ...
  socket.on('thumb', async (data = {}, cb) => {
    const ack = typeof cb === 'function' ? cb : () => { };
    try {
      const cam = String(data.camara_id || '').toLowerCase();
      const uid = String(data.tabla_uid || '').replace(/[^A-Za-z0-9]/g, '').slice(0, 64);
      const b64 = String(data.b64 || data.base64 || '');
      if (!cam || !uid || !b64) return ack({ ok: false, error: 'bad_payload' });

      // Límite defensivo (~200 KB base64)
      if (b64.length > 300 * 1024) return ack({ ok: false, error: 'too_big' });

      const buf = Buffer.from(b64, 'base64');
      const dir = path.join(__dirname, '..', 'public', 'thumbs', cam);
      await fs.promises.mkdir(dir, { recursive: true });
      const file = path.join(dir, `${uid}.jpg`);
      await fs.promises.writeFile(file, buf);
      ack({ ok: true, path: `/thumbs/${cam}/${uid}.jpg` });
    } catch (e) {
      console.error('thumb error:', e);
      ack({ ok: false, error: 'fs_error' });
    }
  });
