...
// ───────── helpers fecha/tipos ─────────
const tzAdjustToSpain = (d) => { if (!(d instanceof Date)) d = new Date(d); return d; };
function toDateOrNull(v) {
  if (v == null) return null;
  if (typeof v === 'number') return new Date(v * 1000); // epoch seconds
  const d = new Date(v); return isNaN(d) ? null : d;
}
function isV2(data) { return data && (data.schema === 'medidas_v2' || data.schema_tag === 'medidas_v2'); }
...
function normalizeIp(ip) { return ip ? String(ip).replace(/^::ffff:/, '').trim() : ''; }
...
function pickRaspberryId(data, socket) {
  const candidates = [
    data?.hostname, data?.device_id, data?.id_raspberry, data?.camara_id,
    hdr(socket, 'x-device-id'), hdr(socket, 'x-rpi-id'), hdr(socket, 'x-hostname'),
  ].filter(v => typeof v === 'string' && v.trim());
  const chosen = candidates[0] || getClientIp(socket);
  return String(chosen).slice(0, 100);
}

// ───────── descabezado: causa derivada si falta ─────────
function deriveDescCausa(d) {
  if (typeof d?.desc_causa === 'string') {
    const v = d.desc_causa.toLowerCase();
    if (v === 'both' || v === 'tip+shape') return 'tip+shape';
    if (v === 'tip') return 'tip';
  ...
  const shape =
    (typeof d?.desc_shape_taper_ratio === 'number' && d.desc_shape_taper_ratio < 0.65) ||
    (typeof d?.desc_shape_area_ratio  === 'number' && d.desc_shape_area_ratio  < 0.70) ||
    (typeof d?.desc_shape_taper_drop  === 'number' && d.desc_shape_taper_drop  > 0.25) ||
    (typeof d?.desc_shape_slope_norm  === 'number' && Math.abs(d.desc_shape_slope_norm) > 0.05);
...
// ───────── wiring de namespaces ─────────
function wireNamespace(nsp, nsName = '/') {
  nsp.on('connection', (socket) => {
    const ip = getClientIp(socket);
    console.log(`[${nsName}] Raspberry Pi conectada: IP ${ip} · ${new Date().toLocaleString()}`);

    socket.on('disconnect', (reason) => {
      console.log(`[${nsName}] Desconectado (${reason})`);
    });

    socket.conn.on('transport', (transport) => {
      console.log(`[${nsName}] transport=${transport.name}`);
    });

    // ---- medidas (Cenital) → upsert + ACK opcional ----
    socket.on('medidas', async (dataArray, cb) => {
      try {
        const arr = Array.isArray(dataArray) ? dataArray : [dataArray];
        let count = 0;
        for (const data of arr) {
          if (!data || !data.ancho_mm || !(data.grosor_lateral_mm ?? data.grosor_mm)) {
          ...
          if (isV2(data)) {
            ...
            // Fallback legacy: mapea mínimos a esquema V2
        ...
        console.info(`[${nsName}] Medida(s) insertada(s): ${count}`);
        if (typeof cb === 'function') cb({ ok: true, count });
      } catch (e) {
        console.error(`[${nsName}] Error procesando medidas:`, e);
        if (typeof cb === 'function') cb({ ok: false, error: String(e?.message || e) });
      }
    });

    // ---- estadísticas de la Pi ----
    ...
    socket.on('message', (data) => {
      console.log(`[${nsName}] Mensaje recibido desde la aplicación Flask:`, data);
    });
  });
}
...
module.exports = (server) => {
  const io = new Server(server, {
    ...
    allowEIO3: true, // por si alguna Pi usa lib antigua
    cors: {
