  ...
  io.on("connection", (socket) => {
    ...
    socket.on("medidas", (dataArray) => {
      ...
      dataArray.forEach((data) => {
        // 1) Guardamos SIEMPRE en la tabla nueva (upsert por tabla_id+frame)
        upsertMedidaCenital(data);

        // 2) Para compatibilidad: solo insertamos en la legacy cuando AÚN no está corregida
        if (!data.corregida) {
          consultarMedidaIdeal(data, (err, medidaIdeal) => {
            if (err) return;
            insertarMedidaLegacy(data, medidaIdeal);
          });
        }
      });
    });
