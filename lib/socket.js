...
// ---------- NUEVO: derivar causa de descabezado si no viene ----------
...
// ACK seguro: no rompe si el cliente no pasó callback o si el ACK lanza.
function ackSafe(ack, payload) {
  try { if (typeof ack === 'function') ack(payload); } catch (e) { console.error('ACK callback failed:', e); }
}

// ---------------- Persistencia medidas (v2) ----------------
async function upsertMedidaCenitalV2(data) {
  ...
  const params = [
    ...
    // grosor real de la lateral (o compat)
...
function wireNamespace(nsp, nsName = '/') {
  nsp.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
      console.log(`[${nsName}] Desconectado de la aplicación Flask en la Raspberry Pi`);
    });

    // ---- medidas desde la cenital (siempre mm reales) — CON ACK ----
    socket.on('medidas', async (dataArray, ack) => {
      try {
        ...
        // ACK de aplicación → el cliente Python deja de encolar/spulear
        ackSafe(ack, { ok: true, n: processed });
      } catch (e) {
        console.error('Error procesando medidas:', e);
        ackSafe(ack, { ok: false, error: String(e && e.message || e) });
      }
    });

    // ---- estadísticas de la Pi — CON ACK ----
    socket.on('estadisticas', async (data = {}, ack) => {
      try {
